...
const SPRITE_OUTPUT_PATH = path.join(__dirname, '../assets/images/sprites/heroicons-outline-sprite.svg');
const HEROICONS_OUTLINE_URL = 'https://raw.githubusercontent.com/tailwindlabs/heroicons/master/optimized/24/outline/';
  ...
  return new Promise((resolve, reject) => {
    https.get(url, (res) => {
      if (res.statusCode !== 200) {
        return reject(new Error(`Failed to download ${name}: ${res.statusCode}`));
      }
...
async function getAllIconNames() {
  ...
  const apiUrl = 'https://api.github.com/repos/tailwindlabs/heroicons/contents/optimized/24/outline';
  return new Promise((resolve, reject) => {
    https.get(apiUrl, { headers: { 'User-Agent': 'node' } }, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode !== 200) {
          return reject(new Error(`Failed to fetch icon list: ${res.statusCode}`));
        }
        try {
          const files = JSON.parse(data);
          const svgs = files.filter(f => f.name.endsWith('.svg')).map(f => f.name);
          resolve(svgs);
        } catch (e) {
          reject(e);
        }
...
function svgToSymbol(svgContent, symbolId) {
  ...
  const body = svgContent.replace(/^[\s\S]*?<svg[^>]*>/, '').replace(/<\/svg>[\s\S]*$/, '');
...
async function buildSprite() {
  ...
  for (const name of sortedIcons) {
    ...
    md += `| ![${iconId}](../assets/images/icons/${name}) | ${iconId} |\n`;
  }
